...
async function CookieMonsta_InitialiseDatabase()
{
    ...
    await DatabaseConnection.query(QueryShowTables, (err, results) =>
    {
        if(err)
        {
        ...
        if(Object.entries(results).length <= 0)
        {
            ...
            await DatabaseConnection.query(QueryCreateUsersCookieTable, (err, results) =>
            {
                if(err)
                {
                    console.log("\x1b[31m*\x1b[0m Query error: " + err + "\x1b[0m");
            ...
            await DatabaseConnection.query(QueryCreateUniqueIndex, (err, results) =>
            {
                if(err)
                {
            ...
            await DatabaseConnection.query(QueryCreatePrefixTable, (err, results) =>
            {
                if(err)
                {
                    console.log("\x1b[31m*\x1b[0m Query error: " + err + "\x1b[0m");
            ...
            await DatabaseConnection.query(QueryCreateBannersTable, (err, results) =>
            {
                if(err)
                {
                    console.log("\x1b[31m*\x1b[0m Query error: " + err + "\x1b[0m");
...
async function CookieMonsta_UserExists(iGuild, iUser)
{
    return new Promise((resolve, reject) =>
    {
        ...
        await DatabaseConnection.query(QueryCheckForUser, [parseInt(iUser), parseInt(iGuild)], (err, results) =>
        {
            if(err)
            {
                reject(err.message);
...
async function CookieMonsta_AddBannerToTable(szPngFile, szNameHex, szStatsHex)
{
    ...
    await DatabaseConnection.query(QueryAddBanner, [szPngFile.trim(), szNameHex.trim(), szStatsHex.trim(), szPngFile.trim(), szNameHex.trim(), szStatsHex.trim()], (err, results) =>
    {
        if(err)
        {
...
async function CookieMonsta_GetUserPoints(iGuild, iUser)
{
    return new Promise((resolve, reject) =>
    {
        ...
        await DatabaseConnection.query(QueryGetUserPoints, [parseInt(iUser), parseInt(iGuild)], (err, results) =>
        {
            if(err)
            {
                reject(err.message);
...
async function CookieMonsta_GetUserLevel(iGuild, iUser)
{
    return new Promise((resolve, reject) =>
    {
        ...
        await DatabaseConnection.query(QueryGetUserLevel, [parseInt(iUser), parseInt(iGuild)], (err, results) =>
        {
            if(err)
            {
                reject(err.message);
...
async function CookieMonsta_UpdatePoints_And_Level(iGuild, iUser, iXPPoints, iLevel)
{
    ...
    await DatabaseConnection.query(QueryUpdatePointsAndLevel, [parseInt(iUser), parseInt(iGuild), (parseInt(iXPPoints) <= 0 || isNaN(iXPPoints) ? 0 : parseInt(iXPPoints)), (parseInt(iLevel) <= 0 || isNaN(iLevel) ? 1 : parseInt(iLevel)), PrimaryKeyValue], (err, results) =>
    {
        if(err)
        {
...
async function CookieMonsta_GetUserCookies(iGuild, iUser)
{
    return new Promise((resolve, reject) =>
    {
        ...
        await DatabaseConnection.query(QueryGetUserCookies, [parseInt(iUser), parseInt(iGuild)], (err, results) =>
        {
            if(err)
            {
                reject(err.message);
...
async function CookieMonsta_SetUserCookies(iGuild, iUser, iCookies)
{
    ...
    await DatabaseConnection.query(QueryUpdateCookies, [parseInt(iUser), parseInt(iGuild), (parseInt(iCookies) <= 0 || isNaN(iCookies) ? 0 : parseInt(iCookies)), PrimaryKeyValue], (err, results) =>
    {
        if(err)
        {
...
async function CookieMonsta_GetUserProfileBanner(iGuild, iUser)
{
    return new Promise((resolve, reject) =>
    {
        ...
        await DatabaseConnection.query(QueryGetUserBanner, [parseInt(iUser), parseInt(iGuild)], (err, results) =>
        {
            if(err)
            {
                reject(err.message);
...
async function CookieMonsta_SetUserProfileBanner(iGuild, iUser, szBannerImageFile)
{
    ...
    await DatabaseConnection.query(QueryUpdateUserBanner, [parseInt(iUser), parseInt(iGuild), szBannerImageFile.trim(), PrimaryKeyValue],(err, results) =>
    {
        if(err)
        {
...
async function CookieMonsta_GetBannerFromDatabase(szFileName)
{
    return new Promise((resolve, reject) =>
    {
        ...
        await DatabaseConnection.query(QueryGetBanner, szPngFile.toString(), (err, results) =>
        {
            if(err)
            {
                reject(err.message);
...
async function CookieMonsta_GetAllBanners()
{
    return new Promise((resolve, reject) =>
    {
        ...
        await DatabaseConnection.query(QueryGetAllBanners, (err, results) =>
        {
            if(err)
            {
                reject(err.message);
