...
module.exports.run = async (bot, message, args) =>
{
    ...
    const szEmbedColor = "#" + (Math.random() * 0xFFFFFF << 0).toString(16);

    const DiscordRichEmbed = new Discord.MessageEmbed()
    .setAuthor("Cookie Monsta | Fidget Spinner", bot.user.displayAvatarURL())
    ...
    .setFooter("Spinner started for: @" + user.username, user.displayAvatarURL())
    .setTimestamp()

    let SpinningMessage = await message.channel.send({ embed: DiscordRichEmbed });
    const iSpinTimeout = (Math.random() * (60 - 5 + 1)) + 5;
    ...
    iSpinnerTimer[user.id] = setInterval(async () =>
    {
        if(iSpinTimeout.toFixed(0) >= 40)
        {
            ...
            .setFooter("Spinner ended for: @" + user.username, user.displayAvatarURL())
            .setTimestamp()
