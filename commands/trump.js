...
module.exports.run = async (bot, message, args) =>
{
    ...
    await getJSON("https://api.tronalddump.io/random/quote", async function(error, response)
    {
        if(error)
        {
        ...
        var GetTrumpTweets = JSON.stringify(response._embedded.source[0].url).replace(/"/g, '');
