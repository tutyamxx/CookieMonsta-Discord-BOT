...
module.exports.run = async (bot, message, szArgs) =>
{
    ...
    {
        return await message.reply(" :no_entry: this parameter can't be empty you scrub :facepalm: ! Type **!lastgame** ``<SteamID 32 here>`` :no_entry:");
    }

    if(!CustomFunctions.isInt(szArgs[0]))
    {
        return await message.reply(" :no_entry: this is not a valid ``SteamID 32`` number :no_entry:");
    }
    
    message.channel.startTyping();

    await getJSON("https://api.opendota.com/api/players/" + parseInt(szArgs[0]) + "/recentMatches", async function (error, response)
    {
        if(error)
        {
        ...
        await getJSON("https://api.opendota.com/api/players/" + parseInt(szArgs[0]), async function (error, response_player)
        {
            if(error)
            {
                return await message.channel.send(":no_entry: Sorry, can't retrieve **Open Dota** data right now... Try later. :disappointed_relieved:  :no_entry:").then(() => message.channel.stopTyping(true)).catch(err => message.channel.stopTyping(true));
