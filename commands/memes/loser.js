...
module.exports.run = async (bot, message, args) =>
{
    ...
    await Promise.all([i1, i2]).then(async (images) =>
    {
        ...
        await images[1].composite(images[0], 586, 532).quality(100).getBuffer(Jimp.MIME_PNG, async (err, buffer) =>
        {
            ...
            await message.channel.send(new Discord.Attachment(buffer, "loser.png")).then(async () => await message.channel.stopTyping(true)).catch(async () => await message.channel.stopTyping(true));
        });
    });
