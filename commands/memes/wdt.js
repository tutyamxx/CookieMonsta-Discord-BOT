...
module.exports.run = async (bot, message, szArgs) =>
{
    ...
    let i2 = await Jimp.read("./BOTImages/WhoDidThis/whodidthis.png");

    const szImageName = "whodidthis.png";

    await Promise.all([i1, i2]).then(async (images) =>
    {
        await images[0].resize(717, 408).quality(100);
        await images[1].composite(images[0], 0, 157).quality(100).getBuffer(Jimp.MIME_PNG, async (err, buffer) =>
        {
            if(err)
            {
            ...
            await message.channel.send(new Discord.Attachment(buffer, szImageName)).then(async () => await message.channel.stopTyping(true)).catch(async () => await message.channel.stopTyping(true));
        });
    });
};
