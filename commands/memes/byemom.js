    ...
    await Promise.all([i1, i2, i3]).then(async (images) =>
    {
        ...
        await images[2].composite(images[0], 532, 9).composite(images[1], 76, 326).quality(100).getBuffer(Jimp.MIME_PNG, async (err, buffer) =>
        {
            ...
            .toBuffer("byemom.png", async (err, buffer2) =>
            {
                ...
                await message.channel.send(new Discord.Attachment(buffer2, "byemom.png")).then(async () => await message.channel.stopTyping(true)).catch(async () => await message.channel.stopTyping(true));
            });
        });
    });
};
