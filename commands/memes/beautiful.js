...
module.exports.run = async (bot, message, args) =>
{
    ...
    await Promise.all([i1, i2, i3]).then(async (images) =>
    {
        ...
        await images[2].composite(images[0], 217, 15).composite(images[1], 206, 213).quality(100).getBuffer(Jimp.MIME_PNG, async (err, buffer) =>
        {
            ...
            await message.channel.send(new Discord.Attachment(buffer, "beautiful.png")).then(async () => await message.channel.stopTyping(true)).catch(async () => await message.channel.stopTyping(true));
        });
    });
};
