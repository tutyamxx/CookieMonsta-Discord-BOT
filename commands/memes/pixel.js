...
module.exports.run = async (bot, message, args) =>
{
    ...
    await Jimp.read(GetUserAvatar).then(async (image) =>
    {
        await image.pixelate(20).quality(100).getBuffer(Jimp.MIME_PNG, async (err, buffer) =>
        {
            ...
            await message.channel.send(new Discord.Attachment(buffer, "pixelated.png")).then(async () => await message.channel.stopTyping(true)).catch(async () => await message.channel.stopTyping(true));
        });
    });
};
